...
<table>
    ...
    <tbody>
        <tr>
			...
			<td><i>none</i></td>
            <td>Begins the file upload process.<br />Note: If <code>autoSubmit</code> is set to <code>true</code> (the default value), there is no need to call <code>submit()</code>. The upload process will begin immediately after the user selects a file.</td>
        </tr>	
        <tr>
