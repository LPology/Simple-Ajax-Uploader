...
Before each upload, our two elements are assigned specific roles using these two functions:
...
For the multiple file upload configuration, we use an additional function, `setProgressContainer(elem)`, which designates an element to be removed from the DOM after the upload is completed.
