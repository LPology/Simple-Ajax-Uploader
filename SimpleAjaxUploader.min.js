...
(a.loaded/a.total)),d.onProgress.call(b,a),l&&(l.style.width=a+"%"))}),c.addEvent(a.upload,"error",function(){b._errorFinish("transfererror","Transfer error during upload",e,"None",l,g,m)}),a.onreadystatechange=function(){4===this.readyState&&(200===this.status?(d.endXHR.call(b,e,f),b._finish(this.responseText,e,l,g,m)):b._errorFinish("servererror","Server error. Status: "+this.status,e,this.responseText,l,g,m))},a.open(d.method.toUpperCase(),h,!0),a.setRequestHeader("X-Requested-With","XMLHttpRequest"),
a.setRequestHeader("X-File-Name",encodeURIComponent(e)),"json"==d.responseType.toLowerCase()&&a.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01"),!0===d.multipart?(h=new FormData,h.append(d.name,this._file),this.log("commencing upload using multipart form"),a.send(h)):(a.setRequestHeader("Content-Type","application/octet-stream"),this.log("commencing upload using binary stream"),a.send(this._file)),this.removeCurrent())},_handleIframeResponse:function(a,b,d,e,f){var g;a.parentNode&&
(g=a.contentWindow?a.contentWindow.document:a.contentDocument&&a.contentDocument.document?a.contentDocument.document:a.contentDocument,g.body&&!1===g.body.innerHTML||(g=g.body.innerHTML,c.remove(a),this._finish(g,b,d,e,f)))},_uploadIframe:function(){var a=this,b=this._settings,d=b.checkProgressInterval,e=this._uploadProgressKey,f=this._progressBar,g=this._progressContainer,l=this._fileSizeBox,k=this._filename,h=this._createIframe(),n=this._createForm(h),p;this._doProgressUpdates&&(p=this._createHiddenInput(b.keyParamName,
e),n.appendChild(p));if(!1===b.startNonXHR.call(this,k))this._disabled&&this.enable(),this._activeUploads--;else{p=b.data;for(var r in p)if(p.hasOwnProperty(r)){var q=this._createHiddenInput(r,p[r]);n.appendChild(q)}n.appendChild(this._file);b.onProgress.call(this,0);f&&(f.style.width="0%");c.addEvent(h,"load",function(){c.contains(a._activeProgressKeys,e)&&c.removeItem(a._activeProgressKeys,e);c.removeEvent(h,"load",arguments.callee);b.endNonXHR.call(a,k);a._handleIframeResponse(h,k,f,l,g);e=null});
a.log("commencing upload using iframe");n.submit();c.remove(n);n=null;a._doProgressUpdates&&(this._activeProgressKeys.push(e),m.setTimeout(function(){a.log("requesting first progress update");a._getUploadProgress(e,f,l,1)},d),this._alterProgressKey());this.removeCurrent()}},_getUploadProgress:function(a,b,d,e){if(a){var f=this,g=this._settings,l=c.newXHR(),k=(new Date).getTime(),k=g.progressUrl+"?progresskey="+encodeURIComponent(a)+"&_="+k;l.onreadystatechange=function(){var h,k=g.checkProgressInterval;
...
null,c.removeItem(f._activeProgressKeys,a),f.log("Progress error. Status: "+this.status+" Response: "+this.responseText);h=null};l.open("GET",k,!0);l.setRequestHeader("X-Requested-With","XMLHttpRequest");l.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01");l.setRequestHeader("Cache-Control","no-cache");l.send();l=null}},_alterProgressKey:function(){this._uploadProgressKey+=c.getUID();57<this._uploadProgressKey.length&&(this._uploadProgressKey=c.getUID()+this._uploadProgressKey.substring(0,
15))},_getUploadProgressKey:function(){var a=this,b=c.newXHR(),d=(new Date).getTime(),d=a._settings.progressUrl+"?getkey="+d;a._uploadProgressKey=null;b.onreadystatechange=function(){var b;4===this.readyState&&(200===this.status?(b=c.parseJSON(this.responseText))&&b.key&&(a._uploadProgressKey=b.key,a.log("upload progress key received. Key: "+b.key)):a.log("Error retrieving progress key. Status: "+this.status+" Server response: "+this.responseText))};b.open("GET",d,!0);b.setRequestHeader("X-Requested-With",
"XMLHttpRequest");b.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01");b.setRequestHeader("Cache-Control","no-cache");b.send();b=null},_checkExtension:function(a){var b=this._settings.allowedExtensions,c=b.length;for(a=a.toLowerCase();c--;)if(b[c].toLowerCase()==a)return!0;return!1},_checkFile:function(){var a=this._filename,b=this._ext,c=this._size;if(!this._file||""===a)return this.log("no file to upload"),!1;if(0<this._settings.allowedExtensions.length&&!this._checkExtension(b))return this.removeCurrent(),
this.log("file extension not permitted"),this._settings.onExtError.call(this,a,b),!1;if(null!==c&&!1!==this._settings.maxSize&&c>this._settings.maxSize)return this.removeCurrent(),this.log(a+" exceeds "+this._settings.maxSize+"K limit"),this._settings.onSizeError.call(this,a,c),!1;this.log("pre-upload file check successful");return!0},submit:function(){this._disabled||(this._activeUploads>=this._settings.maxUploads||1>this._queue.length)||(this._file=this._queue[0],this._XhrIsSupported?(this._filename=
c.getFilename(this._queue[0].name),this._size=Math.round(this._queue[0].size/1024)):this._filename=c.getFilename(this._queue[0].value),this._ext=c.getExt(this._filename),this._checkFile()&&!1!==this._settings.onSubmit.call(this,this._filename,this._ext)&&(this._activeUploads++,(!1===this._settings.multiple||!1===this._settings.queue&&this._activeUploads>=this._settings.maxUploads)&&this.disable(),this._XhrIsSupported?this._uploadXhr():(!1!==this._settings.progressUrl&&null!==this._uploadProgressKey?
this._doProgressUpdates=!0:(this._doProgressUpdates=!1,this.log("no upload progress - progressUrl not defined or progress key not available")),this._uploadIframe())))}};m.ss=c})(window,document);