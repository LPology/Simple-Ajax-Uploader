...
"application/json, text/javascript, */*; q=0.01");b.setRequestHeader("Cache-Control","no-cache");b.send();b=null},_errorMsg:function(a){var b=this._settings.messages,c,e,d="";if("ext"==a){var g=this._settings.allowedExtensions,j=g.length,h,f;c=RegExp("{ext}","g");e=RegExp("{name}","g");for(f=0;f<j;f++)h=g[f].toUpperCase(),f+2===j?h+=" and ":2<j&&f+2<j&&(h+=", "),d+=h;c=b.extError.replace(c,d);c=c.replace(e,this._filename);alert(c)}"size"==a&&(d=this._settings.maxSize+"K",c=RegExp("{size}","g"),e=
RegExp("{name}","g"),c=b.sizeError.replace(c,d),c=c.replace(e,this._filename),alert(c))},_checkFile:function(){if(!this._file||""===this._file.value)return this.log("no file to upload"),!1;if(0<this._settings.allowedExtensions.length&&!this._checkExtension(this._ext))return this.log("File extension not permitted"),this._errorMsg("ext"),!1;if(null!==this._size&&!1!==this._settings.maxSize&&this._size/1024>this._settings.maxSize)return this.log("file exceeds "+this._settings.maxSize+"K limit"),this._errorMsg("size"),
!1;this.log("pre-upload file check is successful");return!0},_checkExtension:function(a){var b=this._settings.allowedExtensions,c=b.length;for(a=a.toLowerCase();c--;)if(b[c].toLowerCase()==a)return!0;return!1},submit:function(){this._disabled||this._activeUploads>=this._settings.maxUploads||(this._file=this._queue[0],this._XhrIsSupported&&this._settings.multiple?(this._filename=null!==this._queue[0].fileName&&void 0!==this._queue[0].fileName?this._queue[0].fileName:this._queue[0].name,this._filename=
ss.getFilename(this._filename)):this._filename=ss.getFilename(this._queue[0].value),this._ext=ss.getExt(this._filename),this._XhrIsSupported&&(this._size=this._queue[0].size),this._checkFile()?!1!==this._settings.onSubmit.call(this,this._filename,this._ext)&&(this._activeUploads++,!1===this._settings.multiple&&this.disable(),this._XhrIsSupported?this._uploadXhr():(!1!==this._settings.progressUrl&&null!==this._uploadProgressKey?this._doProgressUpdates=!0:(this._doProgressUpdates=!1,this.log("no upload progress - progressUrl not defined or progress key not available")),
