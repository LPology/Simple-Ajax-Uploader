...
var ss=ss||{};ss.obj2string=function(a,b){var c=[],d;if("object"!==typeof a)return"";for(d in a)if(a.hasOwnProperty(d)){var e=b?b+"["+d+"]":d,g=a[d];c.push("object"===typeof g?ss.obj2string(g,e):encodeURIComponent(e)+"="+encodeURIComponent(g))}return c.join("&")};ss.extendObj=function(a,b){var c;if("object"!==typeof a||"object"!==typeof b)return!1;for(c in b)b.hasOwnProperty(c)&&(a[c]=b[c])};ss.contains=function(a,b){for(var c=a.length;c--;)if(a[c]===b)return!0;return!1};
...
c,d),a._finish(this.responseText,c,g,e,j)):(a.log("Progress key error. Status: "+this.status+" Response: "+this.responseText),b.onError.call(a,c,"server error",this.responseText)))},h.open("POST",f,!0),h.setRequestHeader("X-Requested-With","XMLHttpRequest"),h.setRequestHeader("X-File-Name",encodeURIComponent(c)),"json"==b.responseType.toLowerCase()&&h.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01"),!0===b.multipart?(f=new FormData,f.append(b.name,a._file),a.log("commencing upload using multipart form"),
h.send(f)):(h.setRequestHeader("Content-Type","application/octet-stream"),a.log("commencing upload using binary stream"),h.send(a._file)),a.removeCurrent())},_handleIframeResponse:function(a,b,c,d,e){var g;a.parentNode&&(g=a.contentWindow?a.contentWindow.document:a.contentDocument&&a.contentDocument.document?a.contentDocument.document:a.contentDocument,g.body&&!1===g.body.innerHTML||(g=g.body.innerHTML,ss.remove(a),this._finish(g,b,c,d,e)))},_uploadIframe:function(){var a=this,b=a._settings,c=b.checkProgressInterval,
d=a._uploadProgressKey,e=a._progressBar,g=a._progressContainer,j=a._fileSizeBox,h=a._filename,f=a._createIframe(),k=a._createForm(f),l;a._doProgressUpdates&&(l=a._createHiddenInput(b.keyParamName,d),k.appendChild(l));if(!1===b.startNonXHR.call(a,h))a._disabled&&a.enable(),this._activeUploads--;else{l=b.data;b.onProgress.call(a,0);e&&(e.style.width="0%");for(var m in l)if(l.hasOwnProperty(m)){var n=a._createHiddenInput(m,l[m]);k.appendChild(n)}k.appendChild(a._file);ss.addEvent(f,"load",function(){ss.contains(a._activeProgressKeys,
d)&&ss.removeItem(a._activeProgressKeys,d);ss.removeEvent(f,"load",arguments.callee);b.endNonXHR.call(a,h);a._handleIframeResponse(f,h,e,j,g);d=null});a.log("commencing upload using iframe");k.submit();ss.remove(k);k=null;a._doProgressUpdates&&(a._activeProgressKeys.push(d),window.setTimeout(function(){a.log("requesting first progress update");a._getUploadProgress(d,e,j,1)},c),a._alterProgressKey());a.removeCurrent()}},_getUploadProgress:function(a,b,c,d){if(a){var e=this,g=e._settings,j=ss.newXHR(),
h=(new Date).getTime(),h=g.progressUrl+"?progresskey="+encodeURIComponent(a)+"&_="+h;j.onreadystatechange=function(){var f,h=g.checkProgressInterval;if(4===this.readyState)if(200===this.status){if((f=ss.parseJSON(this.responseText))&&!0===f.success){d++;g.onUpdateFileSize.call(e,f.size);g.onProgress.call(e,f.pct);b&&(b.style.width=f.pct+"%");c&&f.size&&(c.innerHTML=f.size+"K");if(100>f.pct&&ss.contains(e._activeProgressKeys,a)){if(0===f.pct&&0===f.size&&d>=e._maxUpdateFails){e.log("reached limit of failed progress requests");
return}window.setTimeout(function(){e._getUploadProgress(a,b,c,d);d=a=null},h)}h=null}}else e.log("Progress error. Status: "+this.status+" Response: "+this.responseText);f=null};j.open("GET",h,!0);j.setRequestHeader("X-Requested-With","XMLHttpRequest");j.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01");j.setRequestHeader("Cache-Control","no-cache");j.send();j=null}},_alterProgressKey:function(){this._uploadProgressKey+=ss.getUID()},_getUploadProgressKey:function(){var a=
...
"application/json, text/javascript, */*; q=0.01");b.setRequestHeader("Cache-Control","no-cache");b.send();b=null},_errorMsg:function(a){var b=this._settings.messages,c=RegExp("{name}","g"),d,e="";if("ext"==a){var g=this._settings.allowedExtensions,j=g.length,h,f;d=RegExp("{ext}","g");for(f=0;f<j;f++)h=g[f].toUpperCase(),f+2===j?h+=" and ":2<j&&f+2<j&&(h+=", "),e+=h;d=b.extError.replace(d,e);d=d.replace(c,this._filename);alert(d)}"size"==a&&(e=this._settings.maxSize+"K",d=RegExp("{size}","g"),d=b.sizeError.replace(d,
e),d=d.replace(c,this._filename),alert(d))},_checkFile:function(){if(!this._file||""===this._file.value)return this.log("no file to upload"),!1;if(0<this._settings.allowedExtensions.length&&!this._checkExtension(this._ext))return this.log("File extension not permitted"),this._errorMsg("ext"),!1;if(null!==this._size&&!1!==this._settings.maxSize&&this._size/1024>this._settings.maxSize)return this.log("file exceeds "+this._settings.maxSize+"K limit"),this._errorMsg("size"),!1;this.log("pre-upload file check is successful");
return!0},_checkExtension:function(a){var b=this._settings.allowedExtensions,c=b.length;for(a=a.toLowerCase();c--;)if(b[c].toLowerCase()==a)return!0;return!1},submit:function(){this._disabled||this._activeUploads>=this._settings.maxUploads||(this._file=this._queue[0],this._XhrIsSupported&&this._settings.multiple?(this._filename=null!==this._queue[0].fileName&&void 0!==this._queue[0].fileName?this._queue[0].fileName:this._queue[0].name,this._filename=ss.getFilename(this._filename)):this._filename=
ss.getFilename(this._queue[0].value),this._ext=ss.getExt(this._filename),this._XhrIsSupported&&(this._size=this._queue[0].size),this._checkFile()?!1!==this._settings.onSubmit.call(this,this._filename,this._ext)&&(this._activeUploads++,!1===this._settings.multiple&&this.disable(),this._XhrIsSupported?this._uploadXhr():(!1!==this._settings.progressUrl&&null!==this._uploadProgressKey?this._doProgressUpdates=!0:(this._doProgressUpdates=!1,this.log("no upload progress - progressUrl not defined or progress key not available")),
