...
a);e._finish("","",d,a,b,f,m)}catch(l){e._errorFinish("",l.message,"error",a,b,f,m)}e._sizeFlags[h]&&delete e._sizeFlags.key;k=h=g=b=f=m=null});e.log("Commencing upload using iframe");l.submit();c.remove(l);l=q=null;if(this._opts.progressUrl||this._opts.nginxProgressUrl)this._progKeys.push(h),p.setTimeout(function(){e._getProg(h,d,b,m,1);d=b=m=null},e._opts.checkProgressInterval),this._progKey=c.getUID();this.removeCurrent()}},_getProg:function(a,b,d,f,m){var e=this,k=c.newXHR(),h=(new Date).getTime(),
g,l;a&&(this._opts.nginxProgressUrl?g=e._opts.nginxProgressUrl+"?_="+h:this._opts.progressUrl&&(g=e._opts.progressUrl+"?progresskey="+encodeURIComponent(a)+"&_="+h),l=function(){var g,h,n,s,u;try{if(l&&4===k.readyState){k.onreadystatechange=function(){};l=v;s=k.status;try{u=k.statusText}catch(x){u=""}if(200<=s&&300>s){g=c.parseJSON(k.responseText);m++;if(!1===g){e.log("Error parsing progress response (expecting JSON)");return}if(e._opts.nginxProgressUrl)if("uploading"==g.state)h=g.size,0<h&&(n=Math.round(100*
(g.received/h)),h=Math.round(h/1024));else if("done"==g.state)n=100;else{if("error"==g.state){e.log("Error requesting upload progress: "+g.status);return}}else e._opts.progressUrl&&!0===g.success&&(h=g.size,n=g.pct);n&&(f&&(f.innerHTML=n+"%"),b&&(b.style.width=n+"%"),e._opts.onProgress.call(e,n));h&&!e._sizeFlags[a]&&(e._sizeFlags[a]=1,d&&(d.innerHTML=h+"K"),e._opts.onUpdateFileSize.call(e,h));if(!n&&!h&&m>=e._maxFails){e.log("Failed progress request limit reached");return}100>n&&c.contains(e._progKeys,
a)&&p.setTimeout(function(){e._getProg(a,b,d,f,m);e=a=b=d=f=m=null},e._opts.checkProgressInterval)}else c.removeItem(e._progKeys,a),e.log("Error requesting upload progress: "+s+" "+u);k=h=n=s=u=g=null}}catch(w){e.log("Error requesting upload progress: "+w.message)}},k.onreadystatechange=l,k.open("GET",g,!0),e._opts.nginxProgressUrl&&k.setRequestHeader(e._opts.nginxProgressHeader,a),k.setRequestHeader("X-Requested-With","XMLHttpRequest"),k.setRequestHeader("Accept","application/json, text/javascript, */*; q=0.01"),
k.send())},_checkFile:function(a,b,c){var f=this._opts.allowedExtensions,m=f.length,e=!1;if(0<m){for(b=b.toLowerCase();m--;)if(f[m].toLowerCase()==b){e=!0;break}if(!e)return this.removeCurrent(),this.log("File extension not permitted"),this._opts.onExtError.call(this,a,b),!1}return c&&!1!==this._opts.maxSize&&c>this._opts.maxSize?(this.removeCurrent(),this.log(a+" exceeds "+this._opts.maxSize+"K limit"),this._opts.onSizeError.call(this,a,c),!1):!0},submit:function(){var a,b,d;this._disabled||this._active>=
this._opts.maxUploads||1>this._queue.length||(this._file=this._queue[0],this._XhrOk?(a=c.getFilename(this._file.name),d=Math.round(this._file.size/1024)):a=c.getFilename(this._file.value),b=c.getExt(a),this._checkFile(a,b,d)&&!1!==this._opts.onSubmit.call(this,a,b)&&(this._active++,(!1===this._opts.multiple||!1===this._opts.queue&&this._active>=this._opts.maxUploads)&&this.disable(),this._XhrOk?this._uploadXhr(a,d,this._sizeBox,this._progBar,this._progBox,this._pctBox):this._uploadIframe(a,this._sizeBox,
this._progBar,this._progBox,this._pctBox),this._sizeBox=this._progBar=this._progBox=this._pctBox=null))}};p.ss=c})(window,document);